...
*: end ;
op: 0x04; //MOV_A_L 1 Move A to L no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGA->REGL,jmp0;
*:end;
op: 0x05; //MOV_A_H 1 Move A to H no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGA->REGH,jmp0;
*:end;
op: 0x06; //MOV_L_A 1 Move L to A no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGL->REGA,jmp0;
*:end;
op: 0x07; //MOV_H_A 1 Move H to A no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGH->REGA,jmp0;
*:end;
op: 0x08; //MOV_B_H 1 Move B to H no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGB->REGH,jmp0;
*:end;
op: 0x09; //MOV_B_L 1 Move B to L no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGB->REGL,jmp0;
*:end;
op: 0x0a; //MOV_H_B 1 Move H to B no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGH->REGB,jmp0;
*:end;
op: 0x0b; //MOV_L_B 1 Move L to B no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGL->REGB,jmp0;
*:end ;
...
*: alu(adc),jmp0,swapAD; 
*:end;
op: 0x1f; //ST_L_(X) 1 Write L to mem with X
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGL->ram[mar],jmp0;
*: end;
//
...
//
op: 0x20 ; //AND_A_VAL 2 And A imediate from mem
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->acum,alu(and),jmp0,inc(pc),swapBT; 
*:end;
op: 0x21 ; //AND_D_VAL 2 And B imediate from mem
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->acum,alu(and),inc(pc),jmp0,swapAD,swapBT; 
*:end;
op: 0x22 ; //AND_A_B 1 A and B result in A
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: alu(and),jmp0 ; //B reg on B buss already 
*:end;
op: 0x23 ; //AND_D_C 1 D and C result in D
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: alu(and),jmp0,swapAD  ;
*:end;
op: 0x24 ; //AND_A_H 1 A and H result in A
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGH->acum,alu(and),jmp0,swapBT  ; 
*:end;
op: 0x25 ; //AND_A_L 1 A and L result in A
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGL->acum,alu(and),jmp0,swapBT  ; 
*:end;
op: 0x26 ; //AND_DA_CB 1 AD and CB result in DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: alu(and) ; 
*: alu(and),jmp0,swapAD  ;
*:end;
...
*: alu(slr),jmp0,swapAD; 
*:end;
op: 0x2b ; //OR_DA_CB 1 AD and CB result in DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: alu(or) ; 
*: alu(or),jmp0,swapAD  ;
*:end;
op: 0x2c ; //AND_DA_VAL 3 AD with MEM result in DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: alu(and), swapBT,inc(pc); 
*: alu(and),jmp0,swapAD,swapBT  ;
*:end;
op: 0x2d ; //OR_DA_VAL 3 AD with MEM result in DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: alu(or), swapBT,inc(pc); 
*: alu(or),jmp0,swapAD,swapBT  ;
*:end;
op: 0x2e ; //OR_A_H 1 A or H result in A
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGH->acum,alu(or),jmp0,swapBT  ; 
*:end;
op: 0x2f ; //OR_A_L 1 A or L result in A
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
//
op: 0x30 ; //LD_A_VAL 2 load A immediate 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->REGA,inc(pc),jmp0  ;
*: end;
op: 0x31 ; //LD_B_VAL 2 load B immediate 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->REGB,inc(pc),jmp0  ;
*: end;
op: 0x32 ; //LD_C_VAL 2 load C immediate 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->REGC,inc(pc),jmp0  ;
*: end;
op: 0x33 ; //LD_D_VAL 2 load D immediate 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->REGD,inc(pc),jmp0  ;
*: end;
op: 0x34; //LD_C_(X) 1 Load C from mem with MAR
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[mar]->REGC,jmp0;
*: end;
op: 0x35; //LD_D_(X) 1 Load D from mem with MAR
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[mar]->REGD,jmp0;
*: end;
op: 0x36; //LD_L_(X) 1 Load L from mem with MAR
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[mar]->REGL,jmp0;
*: end;
op: 0x37 ;//LD_SP_VAL 3 Write sp immediate
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->sp[lsb],inc(pc) ;//Write lsb to pc counter
*: ram[pc]->sp[msb],inc(pc),jmp0 ;//WritePC counter and end
*: end ;
op: 0x38; //LD_X_VAL 3 load X imeadiatly from mem
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->mar[lsb],inc(pc);
*: ram[pc]->mar[msb],inc(pc),jmp0;
*: end;
op: 0x39; //LD_Y_VAL 3 load Y imeadiate
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->Y[lsb],inc(pc);
*: ram[pc]->Y[msb], inc(pc),jmp0;  
*: end;
op: 0x3a; //LD_X_(X) 1 load X pointed to by X
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[mar]->mar[lsb],inc(mar);
*: ram[mar]->mar[msb],jmp0;
*: end;
op: 0x3b; //LD_Y_(X) 1 load Y pointed to by X
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[mar]->Y[lsb],inc(mar);
*: ram[mar]->Y[msb],inc(mar),jmp0; 
*: end;
op: 0x3c; //LD_X_(Y) 1 load X pointer to by Y
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[Y]->mar[lsb],inc(Y);
*: ram[Y]->mar[msb],inc(Y),jmp0;
*: end;
op: 0x3d; //LD_Y_(Y) 1 load Y pointed to by Y
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[Y]->Y[lsb],inc(Y);
*: ram[Y]->Y[msb],jmp0;  
*: end;
op: 0x3e ; //LD_H_VAL 2 load H immediate 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->REGH,inc(pc),jmp0  ;
*: end;
op: 0x3f ; //LD_L_VAL 2 load L immediate 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
//
op: 0x40 ; //ADD_DA_VAL 3 Add 16 bit mem to DA 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->acum,alu(add),inc(pc),swapBT; //LSB
*: ram[pc]->acum,alu(adc),jmp0,inc(pc),swapBT,swapAD; //MSB
*: end;
op: 0x41 ; //ADD_A_VAL 2 Add mem location to A
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->acum,alu(add),jmp0,inc(pc),swapBT; 
*:end;
op: 0x42 ; //ADD_D_VAL 2 Add D imediate from mem
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
*: ram[pc]->acum,alu(adc),jmp0,swapAD;  
*:end;
op: 0x46 ; //SUB_A_VAL 2 A - (mem location) result in A 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->acum,alu(sub),jmp0,inc(pc),swapBT;  
*:end;
op: 0x47 ;//SUB_D_VAL 2 D - (mem location) result in D
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
*:end;
op: 0x4a ;//CMP_A_VAL 2 A is compared to value in memeory result in A
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
*:end;
op: 0x4d ; //ADC_A_VAL 2 Add with carry mem location to A store in A
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->acum,alu(adc),jmp0,inc(pc),swapBT; 
*:end;
op: 0x4e ; //ADC_D_VAL 2 Add with carry D imediate from mem store in D
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
//
op: 0x50; //MOV_A_B 1 Move A to B no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGA->REGB,jmp0;
*:end;
op: 0x51; //MOV_A_C 1 Move A to C no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGA->REGC,jmp0;
*:end;
op: 0x52; //MOV_A_D 1 Move A to D no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGA->REGD,jmp0;
*:end;
op: 0x53; //MOV_B_A 1 Move B to A no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGB->REGA,jmp0;
*:end;
op: 0x54; //MOV_B_C 1 Move B to C no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGB->REGC,jmp0;
*:end;
op: 0x55; //MOV_B_D 1 Move B to D no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGB->REGD,jmp0;
*:end;
op: 0x56; //MOV_C_A 1 Move C to A no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGC->REGA,jmp0;
*:end;
op: 0x57; //MOV_C_B 1 Move C to B no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGC->REGB,jmp0;
*:end;
op: 0x58; //MOV_C_D 1 Move C to D no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGC->REGD,jmp0;
*:end;
op: 0x59; //MOV_D_A 1 Move D to A no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGD->REGA,jmp0;
*:end;
op: 0x5a; //MOV_D_B 1 Move D to B no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGD->REGB,jmp0;
*:end;
op: 0x5b; //MOV_D_C 1 Move D to C no flags effected
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGD->REGC,jmp0;
*:end;
...
*: sp[msb]->Y[msb],jmp0 ;
*:end;
op: 0x5e; //MOV_SP_CB 1 Move stack pointer to CB reg
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: sp[lsb]->REGB, ; 
*: sp[msb]->REGC,jmp0 ;
*:end;
...
//
op: 0x60; //ST_A_(X) 1 Write A to mem with MAR
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGA->ram[mar],jmp0;
*: end;
op: 0x61; //ST_A_(Y) 1 Write A to mem with Y
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGA->ram[Y],jmp0;
*: end;
op: 0x62; //ST_B_(X) 1 Write B to mem with X
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGB->ram[mar],jmp0;
*: end;
op: 0x63; //ST_B_(Y) 1 Write B to mem with Y
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGB->ram[Y],jmp0;
*: end;
op: 0x64; //ST_C_(Y) 1 Write C to mem with Y
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGC->ram[Y],jmp0;
*: end;
op: 0x65; //ST_C_(X) 1 Write C to mem with X
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGC->ram[mar],jmp0;
*: end;
op: 0x66; //ST_D_(Y) 1 Write D to mem with Y
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGD->ram[Y],jmp0;
*: end;
op: 0x67; //ST_D_(X) 1 Write D to mem with X
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGD->ram[mar],jmp0;
*: end;
...
*: end;
op: 0x6A; //LD_B_(X) 1 Load B from mem with MAR
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[mar]->REGB,jmp0;
*: end;
op: 0x6b; //LD_A_(X) 1 Load A from mem with MAR
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[mar]->REGA,jmp0;
*: end;
...
//
op: 0x80; //CALL_VAL 3 Unconditional note PC and flags is saved
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
*: end;
op: 0x81; //JMP_VAL 3 Unconditional Jump instruction
// Jump instruction
//
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->pc[lsb],inc(pc) ;//Write lsb to pc counter
...
*: end ;
op: 0x82 ; //JNEQ_VAL 3 Jump if equal than flag set 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
=: inc(pc);
!=: ram[pc]->pc[lsb],inc(pc) ;//Write lsb to pc counter
=: jmp0,inc(pc);
...
op: 0x83 ; //JPZ_VAL 3 Jump if zero flag set
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
!Z:jmp0,inc(pc) ; 
*: end;
op: 0x84; //JMP_(DA) 1 Unconditional Jump instruction with value in DA
// Jump instruction
//
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
*: end ;
op: 0x85 ; //JNZ_VAL 3 Jump if zero flag is not set
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
Z: inc(pc);
!Z: ram[pc]->pc[lsb],inc(pc) ;//Write lsb to pc counter
Z: jmp0,inc(pc);
!Z: ram[pc]->pc[msb],jmp0 ;//WritePC counter and end
*: end;
op: 0x86 ; //JLT_VAL 3 Jump if Less than flag set 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
!LT: jmp0,inc(pc);
*: end;
op: 0x87 ; //JGT_VAL 3 Jump if Greater than flag set
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch 
...
!GT: jmp0,inc(pc);
*: end;
op: 0x88 ; //JEQ_VAL 3 Jump if equal than flag set 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
=: ram[pc]->pc[lsb],inc(pc) ;//Write lsb to pc counter
!=: inc(pc);
=: ram[pc]->pc[msb],jmp0 ;//WritePC counter and end
!=: jmp0,inc(pc);
*: end;
op: 0x89 ; //JPC_VAL 3 Jump if carry than flag set 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
*: end;
//
op: 0x8A ; //JNEG_VAL 3 Jump if negitive flag set 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
!neg: jmp0,inc(pc);
*: end;
op: 0x8b ; //JPOS_VAL 3 Jump if negitive flag set 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
neg: inc(pc);
!neg: ram[pc]->pc[lsb],inc(pc) ;//Write lsb to pc counter
neg: jmp0,inc(pc);
!neg: ram[pc]->pc[msb],jmp0 ;//WritePC counter and end
*: end;
op: 0x8c; //CALL_(DA) 1 with value in DA Unconditional note PC and flags is saved
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGA->pc[lsb],inc(pc) ;//Write lsb to pc counter increament pc
*: pc[msb]->ram[sp],dec(sp);
*: pc[lsb]->ram[sp],dec(sp);
*: REGD->pc[msb],jmp0  ;//WritePC counter Address n-1 was latched
*: end;
op: 0x8e; //RETI 1 Pops pc and restores flags
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: inc(sp);
...
*: alu(xor),jmp0; 
*:end;
op: 0x94 ; //XOR_A_VAL 2 XOR A with memeory 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->acum,alu(xor),jmp0,inc(pc),swapBT;
*:end;
op: 0x95 ; //OR_A_VAL 2 OR mem location to A
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
*: alu(clr),jmp0; 
*:end;
op: 0x9e ; //OR_D_VAL 2 OR mem location to D
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
*:end;
op: 0xa1 ; //XOR_DA_VAL 3 AD with MEM result in DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: alu(xor), swapBT,inc(pc); 
*: alu(xor),jmp0,swapAD,swapBT  ;
*:end;
op: 0xa2 ; //XOR_DA_(X+) 1 AD with MEM pointed to by X result in DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[mar]->acum,alu(xor),swapBT,inc(mar); 
*: ram[mar]->acum,alu(xor),jmp0,swapAD,swapBT,inc(mar);
*:end;
op: 0xa3 ; //SBC_A_VAL 2 A - (mem location) result in A with borrow
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->acum,alu(sbc),jmp0,inc(pc),swapBT;  
*:end;
op: 0xa4 ;//SBC_D_VAL 2 D - (mem location) result in D with borrow
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
*:end;
op: 0xab ;//SUB_DA_VAL 3 DA - mem result in DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->acum,alu(sub),swapBT; 
*: ram[pc]->acum,alu(sbc),jmp0,swapAD,swapBT;  
*:end;
op: 0xac ;//SBC_DA_VAL 3 DA - mem result in DA with borrow
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->acum,alu(sbc),swapBT; 
*: ram[pc]->acum,alu(sbc),jmp0,swapAD,swapBT;  
*:end;
op: 0xad ;//SUB_DA_CB 3 DA - CB result in DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: alu(sub),; 
*: alu(sbc),jmp0,swapAD;  
*:end;
op: 0xae ;//SBC_DA_CB 1 DA - CB result in DA with borrow
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: alu(sbc),; 
*: alu(sbc),jmp0,swapAD;  
*:end;
op: 0xaf ;//CMP_D_VAL 2 D is compared to value in memeory result in D
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
...
*: REGD->Y[msb],jmp0 ;
*:end ;
op: 0xb7; //ST_H_(X) 1 Write H to mem with X
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGH->ram[mar],jmp0;
*: end;
op: 0xb8; //ST_H_(Y) 1 Write H to mem with Y
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGH->ram[Y],jmp0;
*: end;
op: 0xb9; //ST_L_(Y) 1 Write L to mem with Y
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGL->ram[Y],jmp0;
*: end;
op: 0xba; //MOV_X_DA 1 Move X to DA reg
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: mar[lsb]->REGA, ; 
*: mar[msb]->REGD,jmp0 ;
*:end ;
op: 0xbb; //MOV_Y_DA 1 Move Y to DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: Y[lsb]->REGA, ; 
*: Y[msb]->REGD,jmp0 ;
*:end ;
op: 0xbc; //PUSH_HL 1 Push the HL reg pair
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGH->ram[sp],dec(sp) ; //note in this case its the sp
*: REGL->ram[sp],dec(sp),jmp0; 
*: end;
op: 0xbd; //POP_HL 1 pops HL 16 bit value
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: inc(sp);
*: ram[sp]->REGL,inc(sp) ; 
*: ram[sp]->REGH,jmp0;
*: end;
op: 0xbe; //MOV_SP_DA 1 Move stack pointer to DA reg
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: sp[lsb]->REGA, ; 
*: sp[msb]->REGD,jmp0 ;
*:end;
...
*: REGD->REGH,jmp0 ;
*: end
op: 0xc1 ; //LD_HL_VAL 3 load HL immediate 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->REGL,inc(pc) ;
*: ram[pc]->REGH,inc(pc),jmp0  ;
*: end;
op: 0xc2 ; //LD_DA_VAL 3 load DA immediate 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->REGA,inc(pc) ;
*: ram[pc]->REGD,inc(pc),jmp0  ;
*: end;
op: 0xc3 ; //LD_CB_VAL 3 load CB immediate 
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[pc]->REGB,inc(pc) ;
*: ram[pc]->REGC,inc(pc),jmp0  ;
*: end;
...
*: end;
op: 0xd2; //LD_C_(Y) 1 Load C from mem with Y
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[Y]->REGC,jmp0;
*: end;
op: 0xd3; //LD_D_(Y) 1 Load D from mem with Y
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[Y]->REGD,jmp0;
*: end;
op: 0xd4; //LD_L_(Y) 1 Load L from mem with Y
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[Y]->REGL,jmp0;
*: end;
op: 0xd5; //LD_H_(Y) 1 Load H from mem with Y
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[Y]->REGH,jmp0;
*: end;
op: 0xd6; //LD_B_(Y) 1 Load B from mem with Y
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[Y]->REGB,inc(Y),jmp0;
*: end;
op: 0xd7; //ST_D_(Y+) 1 Write D to mem with Y
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: REGD->ram[Y],inc(Y),jmp0;
*: end;
...
// Hex E0 spare
//
op: 0xe0 ; //ADD_DA_(X+) 1 ADD with pointed to by X MEM result in DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[mar]->acum,alu(add),swapBT,inc(mar); 
*: ram[mar]->acum,alu(add),jmp0,swapAD,swapBT,inc(mar);
*:end;
op: 0xe1 ; //ADC_DA_(X+) 1 ADC with pointed to by X MEM result in DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[mar]->acum,alu(adc),swapBT,inc(mar); 
*: ram[mar]->acum,alu(adc),jmp0,swapAD,swapBT,inc(mar);
*:end;
op: 0xe2 ; //SUB_DA_(X+) 1 SUB with pointed to by X MEM result in DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[mar]->acum,alu(sub),swapBT,inc(mar); 
*: ram[mar]->acum,alu(sub),jmp0,swapAD,swapBT,inc(mar);
*:end;
op: 0xe3 ; //SBC_DA_(X+) 1 SBC with pointed to by X MEM result in DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[mar]->acum,alu(sbc),swapBT,inc(mar); 
*: ram[mar]->acum,alu(sbc),jmp0,swapAD,swapBT,inc(mar);
*:end;
op: 0xe4 ; //AND_DA_(X+) 1 AND with pointed to by X MEM result in DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[mar]->acum,alu(and),swapBT,inc(mar); 
*: ram[mar]->acum,alu(and),jmp0,swapAD,swapBT,inc(mar);
*:end;
op: 0xe5 ; //OR_DA_(X+) 1 or with pointed to by X MEM result in DA
*: ram[pc]->instr,inc(pc) ; //basic istruction fetch
*: ram[mar]->acum,alu(or),swapBT,inc(mar); 
*: ram[mar]->acum,alu(or),jmp0,swapAD,swapBT,inc(mar);
*:end;
//
// Hex F0 spare
//